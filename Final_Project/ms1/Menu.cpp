...
namespace sdds
{
	...
	std::ostream& Menu::display(std::ostream& out) const {
		if (this->isEmpty()) {
			...
			if (this->m_title[0]) out << this->m_title << std::endl; // If there is a title display it
	...
	void Menu::add(const char* item) {
		if (item == nullptr) {
			...
			if (this->operator bool() && this->m_itemNumber < MAX_NO_OF_ITEMS) {
